...
import logging
logging.basicConfig(
    level=logging.INFO,  # 设置全局等级
    format='%(asctime)s - %(levelname)s - %(message)s',
)

logger = logging.getLogger(__name__)
logger.setLevel(logging.DEBUG)  # 设置当前模块的日志等级
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: dict):
    ...
    if vec_norm_path.exists():
        ...
        print("no saved normalization found, but normalizing input.")
        env = VecNormalize(
            env,
            training=True,
    ...
    agent = PPO.load(checkpoint_path, 
                ...
                "device": "gpu"  # Force CPU (PPO with MLP on GPU often inefficient)
            }
    )
